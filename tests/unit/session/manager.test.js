...
describe('SessionManager', () => {
    ...
    beforeEach(async () => {
        ...
        tempDir = await setup_1.testHelpers.createTempDir();
        mockConfig.sessionDir = tempDir;
        manager = new manager_1.SessionManager(mockConfig, mockLogger);
    });
    afterEach(async () => {
        await manager.cleanup();
        await setup_1.testHelpers.cleanupTempDir(tempDir);
    });
    describe('createSession', () => {
        ...
        it('should enforce session limits', async () => {
            ...
            if (sessions.length > 0) {
                sessions[0].state.lastActivity = new Date(Date.now() - mockConfig.sessionTimeout - 1000);
            }
        ...
        it('should throw error when cleanup fails and still at limit', async () => {
            ...
            // Act & Assert
            await expect(manager.createSession()).rejects.toThrow(types_1.SessionError);
            await expect(manager.createSession()).rejects.toThrow('Maximum number of sessions reached');
        });
        it('should generate unique IDs for concurrent sessions', async () => {
            ...
            const concurrentManager = new manager_1.SessionManager(highLimitConfig, mockLogger);
            // Act
            const promises = Array.from({ length: 10 }, () => concurrentManager.createSession());
    ...
    describe('loadSession', () => {
        ...
        it('should update last activity when loading session', async () => {
            ...
            // Wait a bit
            await setup_1.testHelpers.wait(10);
            // Act
            const loaded = await manager.loadSession(created.id);
            // Assert
            expect(loaded.state.lastActivity.getTime()).toBeGreaterThan(originalActivity.getTime());
        });
        it('should throw error for non-existent session', async () => {
            // Arrange
            const nonExistentId = 'non-existent-session-id';
            // Act & Assert
            await expect(manager.loadSession(nonExistentId)).rejects.toThrow(types_1.SessionError);
            await expect(manager.loadSession(nonExistentId)).rejects.toThrow(`Session not found: ${nonExistentId}`);
        });
        it('should load session from disk when not in memory', async () => {
            // Arrange
            const sessionId = setup_1.testHelpers.generateTestSessionId();
            ...
            jest
                .spyOn(manager, 'loadSessionFromDisk')
        ...
        it('should handle corrupted session data gracefully', async () => {
            ...
            // Act & Assert
            await expect(manager.loadSession(sessionId)).rejects.toThrow(types_1.SessionError);
        });
    });
    describe('listSessions', () => {
        ...
        it('should filter sessions by tags', async () => {
            ...
            expect(result.items.some((s) => s.metadata.name === 'Session 1')).toBe(true);
            expect(result.items.some((s) => s.metadata.name === 'Session 3')).toBe(true);
        });
        ...
        it('should include correct session status', async () => {
            ...
            result.items.forEach((session) => {
                expect(['active', 'inactive', 'expired', 'error']).toContain(session.status);
            });
        });
    });
    describe('updateSession', () => {
        ...
        it('should throw error for non-existent session', async () => {
            ...
            const nonExistentId = 'non-existent-session-id';
            // Act & Assert
            await expect(manager.updateSession(nonExistentId, { name: 'New Name' })).rejects.toThrow(types_1.SessionError);
        });
        it('should update last activity timestamp', async () => {
            ...
            // Assert
            expect(updated.state.lastActivity.getTime()).toBeGreaterThan(originalActivity.getTime());
        });
    });
    describe('deleteSession', () => {
        ...
        it('should delete existing session', async () => {
            ...
            // Assert
            await expect(manager.loadSession(testSession.id)).rejects.toThrow(types_1.SessionError);
        });
        ...
        it('should remove session from memory and disk', async () => {
            // Arrange
            const deleteFromDiskSpy = jest.spyOn(manager, 'deleteSessionFromDisk');
            // Act
            await manager.deleteSession(testSession.id);
            // Assert
    ...
    describe('addMessage', () => {
        ...
        it('should throw error for non-existent session', async () => {
            ...
            // Act & Assert
            await expect(manager.addMessage('non-existent', message)).rejects.toThrow(types_1.SessionError);
        });
    });
    describe('cleanupExpiredSessions', () => {
        it('should remove expired sessions', async () => {
            ...
            const expiredTime = new Date(Date.now() - mockConfig.sessionTimeout - 1000);
            ...
            await expect(manager.loadSession(activeSession.id)).resolves.toBeDefined();
        });
    ...
    describe('cleanup', () => {
        it('should persist all active sessions', async () => {
            ...
            const persistSpy = jest.spyOn(manager, 'persistSession');
            // Act
            await manager.cleanup();
            // Assert
    ...
    describe('edge cases', () => {
        it('should handle concurrent session creation', async () => {
            ...
            const concurrentManager = new manager_1.SessionManager(highLimitConfig, mockLogger);
            // Act
            const promises = Array.from({ length: 20 }, (_, i) => concurrentManager.createSession({ name: `Concurrent ${i}` }));
        ...
        it('should handle session operations during cleanup', async () => {
            ...
            await expect(Promise.all([cleanupPromise, loadPromise])).resolves.toBeDefined();
        });
        it('should handle invalid session IDs gracefully', async () => {
            // Act & Assert
            await expect(manager.loadSession('')).rejects.toThrow();
            await expect(manager.loadSession(null)).rejects.toThrow();
            await expect(manager.loadSession(undefined)).rejects.toThrow();
        });
    });
});
