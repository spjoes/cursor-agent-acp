...
describe('CursorToolsProvider', () => {
    ...
    beforeEach(() => {
        ...
        provider = new cursor_tools_1.CursorToolsProvider(mockConfig, mockLogger, mockCliBridge);
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    describe('constructor', () => {
        test('should initialize with default configuration', () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith('CursorToolsProvider initialized', expect.objectContaining({
                enabled: true,
        ...
        test('should handle disabled configuration', () => {
            ...
            const disabledProvider = new cursor_tools_1.CursorToolsProvider(disabledConfig, mockLogger, mockCliBridge);
            expect(disabledProvider.getTools()).toEqual([]);
            expect(mockLogger.debug).toHaveBeenCalledWith('Cursor tools disabled by configuration');
        });
    });
    describe('getTools', () => {
    ...
    describe('searchCodebase', () => {
        ...
        test('should handle search failure', async () => {
            ...
            expect(mockLogger.error).toHaveBeenCalledWith('Failed to search codebase', expect.objectContaining({
                query: 'nonexistent',
            }));
        });
...
src/file2.ts:25:1:const test = () => {`,
    ...
    describe('applyCodeChanges', () => {
        ...
        test('should handle disabled code modification', async () => {
            ...
            const restrictedProvider = new cursor_tools_1.CursorToolsProvider(restrictedConfig, mockLogger, mockCliBridge);
    ...
    describe('error handling', () => {
        test('should handle CLI bridge errors gracefully', async () => {
            mockCliBridge.executeCommand.mockRejectedValue(new Error('CLI not found'));
            const tools = provider.getTools();
            const searchTool = tools.find((t) => t.name === 'search_codebase');
            const result = await searchTool.handler({
                query: 'test',
...
//# sourceMappingURL=cursor-tools.test.js.map