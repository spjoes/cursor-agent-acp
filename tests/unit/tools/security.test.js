...
describe('Tool System Security', () => {
    ...
    beforeAll(async () => {
        ...
        tempDir = await fs_1.promises.mkdtemp(path.join(os.tmpdir(), 'cursor-security-test-'));
        allowedDir = path.join(tempDir, 'allowed');
        forbiddenDir = path.join(tempDir, 'forbidden');
        await fs_1.promises.mkdir(allowedDir, { recursive: true });
        await fs_1.promises.mkdir(forbiddenDir, { recursive: true });
        // Create test files
        await fs_1.promises.writeFile(path.join(allowedDir, 'safe.txt'), 'Safe content');
        await fs_1.promises.writeFile(path.join(forbiddenDir, 'secret.txt'), 'Secret content');
    });
    afterAll(async () => {
        try {
    ...
    describe('Filesystem Security', () => {
        ...
        test('should enforce file size limits', async () => {
            ...
            await fs_1.promises.writeFile(largePath, largeContent);
            const call = {
                id: 'large-file',
                name: 'read_file',
                parameters: {
        ...
        test('should prevent symlink exploitation', async () => {
            ...
            try {
                ...
                await fs_1.promises.symlink(forbiddenDir, symlinkPath);
                const call = {
                    id: 'symlink-exploit',
                    name: 'list_directory',
                    parameters: {
        ...
        test('should validate file extension restrictions', async () => {
            ...
            const restrictedRegistry = new registry_1.ToolRegistry(restrictedConfig, mockLogger);
            // Try to read a non-allowed file type
            const executablePath = path.join(allowedDir, 'script.sh');
            await fs_1.promises.writeFile(executablePath, '#!/bin/bash\necho "test"');
            const call = {
                id: 'restricted-ext',
                name: 'read_file',
                parameters: {
    ...
    describe('Terminal Security', () => {
        ...
        test('should enforce process limits', async () => {
            ...
            const results = await Promise.all(calls.map((call) => registry.executeTool(call)));
    ...
    describe('Access Control and Permissions', () => {
        test('should respect disabled tool providers', async () => {
            ...
            const disabledRegistry = new registry_1.ToolRegistry(disabledConfig, mockLogger);
            ...
            expect(tools.filter((t) => t.name.startsWith('read_') || t.name.startsWith('execute_'))).toHaveLength(0);
        });
        ...
        test('should log security violations', async () => {
            ...
            const errorCall = mockLogger.error.mock.calls.find((call) => call[0].includes('Failed to') ||
    ...
    describe('Rate Limiting and Resource Protection', () => {
        test('should handle concurrent tool calls safely', async () => {
            ...
            const promises = concurrentCalls.map((call) => registry.executeTool(call));
        ...
        test('should prevent resource exhaustion', async () => {
            ...
            const sessionPromises = sessionCalls.map((call) => registry.executeTool(call));
            const sessionResults = await Promise.all(sessionPromises);
            const successfulSessions = sessionResults.filter((r) => r.success).length;
            expect(successfulSessions).toBeLessThanOrEqual(mockConfig.tools.terminal.maxProcesses);
        });
